...
remove_stop_word_tokens = [
    'and', 'an', 'the', 'a', 'he', 'she', 'it', 'we', 'you', 'they', 'my',
    'mine', 'your', 'yours', 'their', 'theirs', 'our', 'ours', 'his', 'her',
    ...
    'whom', 'which', 'what', 'where', 'when', 'why', 'how', 'whoever',
    'whichever', 'whenever', 'wherever', 'however', 'every', 'each', 'any',
    ...
    "various", "via", "within", "about", "as", "because", "but", "can",
...
class SrlExtractJson:
    ...
    def extract_values(self, json_objects):
        return [str(item) for obj in json_objects for k, v in obj.items() for item in (v if isinstance(v, list) else [v])]
    ...
    def doit(self, input_string, max_length=10, seed=0, remove_stop_words=False):
        random.seed(seed)
        input_string = input_string.lower()
        ...
        values = [re.sub(r" {2,}", " ", value.strip()) for value in values]
        values = sorted(set(values), key=len, reverse=True)
        values = sorted(set(itertools.chain.from_iterable(v.split(',') for v in values)), key=len, reverse=True)
        values = [re.sub(r" {2,}", " ", value.strip()) for value in values]
        ...
        for i, v in enumerate(values, start=1):
            print(f"{i}: {v}")
        # Randomly select 'max_length' values for final output
        selected_values = random.sample(values, min(len(values), max_length))
